...
public:
  ...
  uint16_t fwd_dport_min; // minumum value for destination port in the forward direction
  uint16_t fwd_dport_max; // maximum value for destination port in the forward direction
  uint16_t rev_sport_min; // minumum value for source port in the reverse direction
  uint16_t rev_sport_max; // maximum value for source port in the reverse direction
  ...
  CE_data *CE;                    // a pointer to the currently simulated CE's data.
...
public:
  ...
  struct in6_addr *bg_addr; // The ipv6 address that will be used for background traffic (This will be the dest. addr. in case of forward and the src. addr. in case of reverse)
  senderCommonParameters(uint16_t ipv6_frame_size_, uint16_t ipv4_frame_size_, uint32_t frame_rate_, uint16_t test_duration_,
                         uint32_t n_, uint32_t m_, uint64_t hz_, uint64_t start_tsc_, CE_data *CE_array_, uint32_t num_of_CEs_, uint16_t num_of_port_sets_,
                         uint16_t num_of_ports_, struct in6_addr *dmr_ipv6_, struct in6_addr *bg_addr_);
};
