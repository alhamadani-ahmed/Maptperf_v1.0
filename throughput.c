...
Throughput::Throughput()
{
  ...
  fwd_dport_min = WIDE_DPORT_MIN;                                                                                       // default value: use maximum range recommended by RFC 4814
  fwd_dport_max = WIDE_DPORT_MAX;                                                                                       // default value: use maximum range recommended by RFC 4814
  ...
  rev_sport_min = WIDE_SPORT_MIN;                                                                                       // default value: use maximum range recommended by RFC 4814
  rev_sport_max = WIDE_SPORT_MAX;                                                                                       // default value: use maximum range recommended by RFC 4814
  CE = NULL;                                                                                                            // a pointer to the currently simulated CE data
  bmr_ipv6_prefix = {{0x20, 0x01, 0x0d, 0xb8, 0x00, 0xce, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}}; // The BMR’s Rule IPv6 Prefix of the MAP address
  bmr_ipv6_prefix_length = 51;                                                                                          // The BMR's Rule IPv6 Prefix length
  bmr_ipv4_prefix_length = 24;                                                                                          // The BMR's IPv4 prefix length
  bmr_EA_length = 13;                                                                                                   // the number of the BMR's EA bits
  dmr_ipv6_prefix = {{0x00, 0x64, 0xff, 0x9b, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}}; // The IPv6 prefix that will be added by DMR to the public IPv4 address
  dmr_ipv6_prefix_length = 96;                                                                                          // The DMR's IPv6 prefix length
  dmr_ipv6 = IN6ADDR_ANY_INIT;                                                                                          // The DMR's IPv6 address
};
...
int Throughput::readConfigFile(const char *filename)
{
  ...
  for (line_no = 1; fgets(line, LINELEN + 1, f); line_no++)
  {
    ...
    else if ((pos = findKey(line, "BMR-IPv4-Prefx-Length")) >= 0)
    {
      ...
      {
        std::cerr << "Input Error: 'bBMR-IPv4-Prefix-Length' must be >= 0 and <= 32." << std::endl;
        return -1;
      }
    }
...
int Throughput::init(const char *argv0, uint16_t leftport, uint16_t rightport)
{
  ...
  // prepare some values for random selection of EA bits value. Hence, calculating the ipv4 suffix and the psid (i.e, the port range within the port set)
  ...
  num_of_suffixes = pow(2.0, bmr_ipv4_suffix_length);
  ...
  std::cout << "Info: Building MAP CE array...." << std::endl;
  buildMapArray();
  std::cout << "Info: MAP CE array: built successfully." << std::endl;

  // Construct the DMR ipv6 address (It will be the destination address in the forward direction and be used to derive the IPv4 source address in the reverse direction)
...
// builds an array of simulated CEs. Each element represents the data of a simulated CE.
// The number of elements (i.e. CEs) is specified as a configuration file parameter
void Throughput::buildMapArray()
{

  // prepare random number infrastructure (to randomly select a psid)
  thread_local std::random_device rd;                                   // Will be used to obtain a seed for the random number engines
  thread_local std::mt19937_64 gen_port_set_id(rd());                   // Standard 64-bit mersenne_twister_engine seeded with rd()
  std::uniform_int_distribution<int> uni_psid(0, num_of_port_sets - 1); // uniform distribution in [0, num_of_port_sets-1]

  // randomly select an ipv4 suffix
  thread_local std::random_device rd2;                                       // Will be used to obtain a seed for the random number engines
  thread_local std::mt19937_64 gen_ipv4_suffix(rd2());                       // Standard 64-bit mersenne_twister_engine seeded with rd()
  std::uniform_int_distribution<int> uni_ip4_suffix(1, num_of_suffixes - 1); // uniform distribution in [1, num_of_suffixes-1]

  uint64_t end_user_ipv6_prefix = 0;
  uint64_t interface_id = 0;
  uint32_t map_addr_chksum = 0;
  for (int curr = 0; curr < num_of_CEs; curr++)
  {
    ...
    CE[curr].psid = uni_psid(gen_port_set_id);
    bmr_ipv4_suffix = uni_ip4_suffix(gen_ipv4_suffix);
    ...
    map_addr_chksum = ((map_addr_chksum & 0xffff0000) >> 16) + (map_addr_chksum & 0xffff);
...
int send(void *par)
{
  ...
  struct in6_addr *bg_addr = cp->bg_addr;
  ...
  //(more specifically, the uncomplemented checksum start value after calculating it by the DPDK rte_ipv4_cksum() and rte_ipv6_udptcp_cksum() functions
  // when creating the template packets)
  uint32_t zero_src_ipv4, zero_dst_ipv4;
  struct in6_addr zero_src_ipv6, zero_dst_ipv6;

  // The src_ipv4 will be set later as the ipv4 address that is embedded in the dmr_ipv6 address.
  ...
  /// the dst_ipv4 must be 0.0.0.0 , first, in order for the ipv4 header checksum to be calculated correctly by The rte_ipv4_cksum(ip_hdr)
  // which gives correct fg_ipv4_chksum_start. So, we can add the generated dst_ipv4 to it later and consequently calculate correct checksum
  ...
  uint32_t *src_ipv4 = &zero_src_ipv4; // This would be set without change during testing in the reverse direction. Please, see below
                                       // It will represent the ipv4 address of the Tester's right interface
  ...
  struct in6_addr *src_ipv6 = &zero_src_ipv6; // This would be variable during testing in the forward direction.
                                              // It will represent the simulated CE (MAP address) and is merely specified inside the sending loop
                                              // using the CE_array
  struct in6_addr *dst_ipv6 = (direction == "forward" ? dmr_ipv6 : &zero_dst_ipv6); // This would be set without change during testing in the
                                                                                    // forward direction as it will represent the dmr_ipv6 address.

  // These addresses are for the background traffic
  struct in6_addr *src_bg = (direction == "reverse" ? bg_addr : &zero_src_ipv6); //if diection is forward, it will be the simulated CE's MAP addr later in the sending loop 
  struct in6_addr *dst_bg = (direction == "forward" ? bg_addr : &zero_dst_ipv6); //if diection is reverse, it will be the simulated CE's MAP addr later in the sending loop
  

  uint16_t sport_min = (direction == "reverse" ? preconfigured_port_min : WIDE_SPORT_MIN);
  uint16_t sport_max = (direction == "reverse" ? preconfigured_port_max : WIDE_SPORT_MAX);
  uint16_t dport_min = (direction == "forward" ? preconfigured_port_min : WIDE_DPORT_MIN);
  uint16_t dport_max = (direction == "forward" ? preconfigured_port_max : WIDE_DPORT_MAX);
  ...
  uint32_t tester_right_ipv4addr; // This should be out of the next if block (i.e., not local) because it will be referenced by a pointer

  // setting the source ipv4 address of the reverse direction to the ipv4 address of the tester right interface
  if (direction == "reverse")
  {
    tester_right_ipv4addr = (uint32_t)dmr_ipv6->s6_addr[12]; 
    for (int i = 13; i < 16; i++)
    {
      tester_right_ipv4addr = tester_right_ipv4addr << 8;
      tester_right_ipv4addr |= dmr_ipv6->s6_addr[i];
    }
    src_ipv4 = &tester_right_ipv4addr;
  ...
  uint16_t *fg_ipv4_chksum[N]; // pointer to the IPv4 header checksum
 ...
 for (i = 0; i < N; i++)
  {
    ...
    if (direction == "reverse")
    {
      ...
      fg_dst_ipv4[i] = (uint32_t *)(pkt + 30); // The destination ipv4 should be manipulated in the sending loop as it will be the BMR-ipv4-prefix + suffix (i.e. changing each time)
    ...
    { //"forward"
      ...
      pkt = rte_pktmbuf_mtod(fg_pkt_mbuf[i], uint8_t *); // Access the Test Frame in the message buffer
      fg_src_ipv6[i] = (struct in6_addr *)(pkt + 22);    // The source address should be manipulated as it will be the MAP address (i.e. changing each time)
    ...
    if (direction == "forward") // The source address should be manipulated as it will be the MAP address (i.e. changing each time)
      bg_src_ipv6[i] = (struct in6_addr *)(pkt + 22);
    // however, the destination address will not be manipulated as it will be the bg_addr (i.e., tester-right-ipv6)
    if (direction == "reverse") // The destination address should be manipulated as it will be the MAP address (i.e. changing each time)
      bg_dst_ipv6[i] = (struct in6_addr *)(pkt + 38);
    // however, the source address will not be manipulated as it will be the bg_addr (i.e., tester-right-ipv6)
    bg_udp_sport[i] = (uint16_t *)(pkt + 54);
    bg_udp_dport[i] = (uint16_t *)(pkt + 56);
    bg_udp_chksum[i] = (uint16_t *)(pkt + 60);
    bg_udp_chksum_start = ~*bg_udp_chksum[i]; // save the uncomplemented UDP checksum value (same for all values of "i")
  }
  ...
  // No change will happen to sport_min, sport_max, dport_min, and dport_max in case of the background packets.
  
  if (var_sport == 1)
  {
    sport = sport_min;
    curr_sport_for_bg = sport_min;
  }
  if (var_sport == 2)
  {
    sport = sport_max;
    curr_sport_for_bg = sport_max;
  }
  if (var_dport == 1)
  {
    dport = dport_min;
    curr_dport_for_bg = dport_min;
  }
  if (var_dport == 2)
  {
    dport = dport_max;
    curr_dport_for_bg = dport_max;
  }
  ...
  { // Main cycle for the number of frames to send
    ...
    else
    {
      ...
      if (direction == "forward")
      {
        *bg_src_ipv6[i] = CE_array[current_CE].map_addr; //set it with the MAP address
        chksum += CE_array[current_CE].map_addr_chksum; //and add its checksum to the UDP checksum

        // In case that the last packet is foreground in the forward direction, then the sport_min, sport_max, and sport will be changed and we should restore the bg's values.
        sport_min = WIDE_SPORT_MIN;
        sport_max = WIDE_SPORT_MAX;
        if (var_sport == 1 || var_sport == 2)
          sport = curr_sport_for_bg; // restore the last used sport in the bg traffic to start over from it (useful when increment or decrement; useless when random)
      }
      if (direction == "reverse")
      {
        *bg_dst_ipv6[i] = CE_array[current_CE].map_addr; //set it with the MAP address
        chksum += CE_array[current_CE].map_addr_chksum; //and add its checksum to the UDP checksum

        // In case that the last packet is foreground in the reverse direction, then the dport_min, dport_max, and dport will be changed and we should restore the bg's values.
        dport_min = WIDE_DPORT_MIN;
        dport_max = WIDE_DPORT_MAX;
        if (var_dport == 1 || var_dport == 2)
          dport = curr_dport_for_bg; // restore the last used dport in the bg traffic to start over from it (useful when increment or decrement; useless when random)          }
      }
    }
    ...
    if (var_sport)
    {
      ...
      *udp_sport = htons(sp); // set the source port 
      chksum += *udp_sport; // and add it to checksum
    }
    if (var_dport)
    {
      // dport is varying
      ...
      *udp_dport = htons(dp); // set the destination port 
      chksum += *udp_dport; // and add it to checksum
    }
    ...
    { // background
      if (direction == "forward")
        curr_sport_for_bg = sport; // save the current sport of the bg as a starting point for the later use because it could be changed if the next frame is a foreground one.
      else
        curr_dport_for_bg = dport; // save the current dport of the bg as a starting point for the later use because it could be changed if the next frame is a foreground one.
    }
...
void Throughput::measure(uint16_t leftport, uint16_t rightport)
{
  ...
  { // Left to right direction is active
    ...
    senderParameters spars(&scp, pkt_pool_left_sender, leftport, "forward", (ether_addr *)dut_left_mac, (ether_addr *)tester_left_mac, 
  ...
  { // Right to Left direction is active
    ...
    // Initialize the parameter class instance
    senderParameters spars(&scp, pkt_pool_right_sender, rightport, "reverse", (ether_addr *)dut_right_mac, (ether_addr *)tester_right_mac,
  ...
  rte_free(CE); // release the CEs data memory
...
senderCommonParameters::senderCommonParameters(uint16_t ipv6_frame_size_, uint16_t ipv4_frame_size_, uint32_t frame_rate_, uint16_t test_duration_,
                                               uint32_t n_, uint32_t m_, uint64_t hz_, uint64_t start_tsc_, CE_data *CE_array_, uint32_t num_of_CEs_,
                                               uint16_t num_of_port_sets_, uint16_t num_of_ports_, struct in6_addr *dmr_ipv6_, struct in6_addr *bg_addr_)
{
