#!/bin/bash
#Parameters
impl="MAPTPERF-PDVasTP-SELF-10G-1M_CEs-Fg100-PromiscIs0-fto10" # name of the tested implementation (used for logging)
...
fto=10 # frame timeout in milliseconds: 0 means pdv, >0 means special throughput measurement with individual frame timeout
...
echo "Global Timeout value (sec): $to"  >> ratetest.log
echo "Frame Timeout value (sec): $fto"  >> ratetest.log
...
do
	...
	do 
		...
		echo "Command line is: ./build/maptperf-pdv $fs $r $xpts $to $n $m $fto"
		# Execute the test program
		./build/maptperf-pdv $fs $r $xpts $to $n $m $fto> temp.out 2>&1
...
dirname="$res_dir/$(date +$impl'-'$dir'-'$fs'-'$n'-'$m'-'$xpts'-'$to'-'$fto'-'$e'-%F-%H%M')"
mkdir -p $dirname
