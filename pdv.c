...
int sendPdv(void *par)
{
  ...
  uint64_t frames_to_send = test_duration * frame_rate; // Each active sender sends this number of packets
  ...
  *send_ts = snd_ts; // return the address of the array to the caller function
  ...
  current_CE = 0;
  uint16_t dst_ipv4_chksum = 0; // The checksum value of the varying destination ipv4 address in the reverse direction of the foreground frame
...
void Pdv::measure(uint16_t leftport, uint16_t rightport)
{
  ...
  { // Left to right direction is active
    ...
    senderParametersPdv spars(&scp, pkt_pool_left_sender, leftport, "forward", (ether_addr *)dut_left_mac, (ether_addr *)tester_left_mac, fwd_var_sport, fwd_var_dport, fwd_dport_min, fwd_dport_max, &left_send_ts);
  ...
  { // Right to Left direction is active
    ...
    // initialize the parameter class instance
    senderParametersPdv spars(&scp, pkt_pool_right_sender, rightport, "reverse", (ether_addr *)dut_right_mac, (ether_addr *)tester_right_mac,
