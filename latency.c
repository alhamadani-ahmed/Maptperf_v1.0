...
int sendLatency(void *par)
{
  ...
  uint32_t *src_ipv4 = &zero_src_ipv4; // This would be set without change during testing in the reverse direction. Please, see below
                                       // It will represent the the ipv4 address of the Tester's right interface
  ...
  struct in6_addr *src_bg = (direction == "reverse" ? bg_addr : &zero_src_ipv6);
  struct in6_addr *dst_bg = (direction == "forward" ? bg_addr : &zero_dst_ipv6);
  ...
  for (i = 0; i < N; i++)
  {
    ...
    if (direction == "reverse")
    {
      ...
      fg_dst_ipv4[i] = (uint32_t *)(pkt + 30); // The destination ipv4 should be manipulated in the sending loop as it will be the BMR-ipv4-prefix + suffix (i.e. changine each time)
    ...
    fg_udp_chksum_start = ~*fg_udp_chksum[i]; // save the uncomplemented checksum value (same for all values of "i")
    ...
    bg_udp_chksum_start = ~*bg_udp_chksum[i]; // save the uncomplemented checksum value (same for all values of "i")
  }
  ...
  for (int i = 0; i < num_of_tagged; i++)
    ...
    else
    {
      ...
      pkt = rte_pktmbuf_mtod(latency_frames[i], uint8_t *); // Access the Test Frame in the message buffer
      if (direction == "forward")                           // The source address should be manipulated as it will be the MAP address (i.e. changing each time)
        lat_bg_src_ipv6[i] = (struct in6_addr *)(pkt + 22);
      // however, the destination address will not be manipulated as it will be the bg_addr (i.e., tester-right-ipv6)
      if (direction == "reverse") // The destination address should be manipulated as it will be the MAP address (i.e. changing each time)
        lat_bg_dst_ipv6[i] = (struct in6_addr *)(pkt + 38);
      // however, the source address will not be manipulated as it will be the bg_addr (i.e., tester-right-ipv6)
  ...
  // The sport and dport values are initialized according to the wide range of values.
  ...
  { // Main cycle for the number of frames to send
    ...
    if (unlikely(sent_frames == send_next_latency_frame))
    {
      ...
      if (sent_frames % n < m)
      {
        ...
        chksum = (uint16_t) ~*lat_fg_udp_chksum[latency_timestamp_no]; // take the value of the uncomplemented checksum
        ...
        if (direction == "reverse")
        {
          ip_chksum = lat_fg_ipv4_chksum_start; // take the value of the uncomplemented checksum
      ...
      else
      {
        ...
        chksum = (uint16_t) ~*lat_bg_udp_chksum[latency_timestamp_no]; // take the value of the uncomplemented checksum
        ...
        if (direction == "forward")
        {
          *lat_bg_src_ipv6[latency_timestamp_no] = CE_array[current_CE].map_addr; //set it with the MAP address
        ...
        if (direction == "reverse")
        {
          *lat_bg_dst_ipv6[latency_timestamp_no] = CE_array[current_CE].map_addr; //set it with the MAP address
          chksum += CE_array[current_CE].map_addr_chksum; //and add its checksum to the UDP checksum
...
void Latency::measure(uint16_t leftport, uint16_t rightport)
{
  ...
  { // Left to right direction is active
    ...
    senderParametersLatency spars(&scp, pkt_pool_left_sender, leftport, "forward", (ether_addr *)dut_left_mac, (ether_addr *)tester_left_mac, fwd_var_sport, fwd_var_dport, fwd_dport_min, fwd_dport_max, left_send_ts);
  ...
  { // Right to Left direction is active
    ...
    // initialize the parameter class instance
    senderParametersLatency spars(&scp, pkt_pool_right_sender, rightport, "reverse", (ether_addr *)dut_right_mac, (ether_addr *)tester_right_mac,
...
senderCommonParametersLatency::senderCommonParametersLatency(uint16_t ipv6_frame_size_, uint16_t ipv4_frame_size_, uint32_t frame_rate_, uint16_t test_duration_,
                                                             ...
                                                             uint16_t first_tagged_delay_, uint16_t num_of_tagged_) : senderCommonParameters(ipv6_frame_size_, ipv4_frame_size_, frame_rate_, test_duration_, n_, m_, hz_, start_tsc_, CE_array_, num_of_CEs_,
                                                                                                                                             num_of_port_sets_, num_of_ports_, dmr_ipv6_, bg_addr_)
{
...
senderParametersLatency::senderParametersLatency(class senderCommonParameters *cp_, rte_mempool *pkt_pool_, uint8_t eth_id_, const char *direction_,
                                                 ...
                                                 uint16_t preconfigured_port_min_, uint16_t preconfigured_port_max_, uint64_t *send_ts_) : senderParameters(cp_, pkt_pool_, eth_id_, direction_,
